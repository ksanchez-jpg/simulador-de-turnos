...
def run_calculation(use_actual_personnel):
    ...
    try:
        ...
        # --- ✅ Ajuste para que todos los operadores queden iguales ---
        horas_totales_por_operador = 128  # exacto en 3 semanas
        st.info(
            f"Cada operador trabajará exactamente {horas_totales_por_operador} horas en 3 semanas "
            f"(≈{horas_totales_por_operador/3:.2f} horas promedio semanal), garantizando balance uniforme."
        )
            ...
            for dia in range(dias_a_programar):
                ...
                for j in range(num_empleados_este_turno):
                    ...
                    if use_actual_personnel and j in indices_descanso:
                        ...
                        horas_trabajadas_por_operador[global_op_idx] += (12 if semana == 0 else 8)
                    elif horas_trabajadas_por_operador[global_op_idx] >= horas_totales_por_operador:
                        dia_programacion.append("Descanso")
                    else:
                        if j in indices_descanso:
                            ...
                            if cantidad_turnos == "Mix" and semana == 0 and i == 2:
                                ...
                                if cantidad_turnos == "Mix" and semana == 0:
                                    dia_programacion.append(f"Turno {turnos_semanales[j % len(turnos_semanales)]}")
                                    horas_trabajadas_por_operador[global_op_idx] += 12
                                else:
                                    dia_programacion.append(f"Turno {turnos_semanales[j % len(turnos_semanales)]}")
                                    horas_trabajadas_por_operador[global_op_idx] += 8
